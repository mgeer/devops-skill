...
while IFS= read -r file; do
  # 允许的范围：overlays/dev/kustomization.yaml
  if echo "${file}" | grep -qE '^services/.+/overlays/dev/kustomization\.yaml$'; then
    ...
    VIOLATIONS="${VIOLATIONS}\n  VIOLATION: ${file} — non-dev overlay modified via direct push"
...
if [ -n "${VIOLATIONS}" ]; then
  ...
  echo -e "${VIOLATIONS}"
  echo ""
  echo "==> Direct push should only modify overlays/dev/ image tags."
